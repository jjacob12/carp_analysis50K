...
```{r DimplotSCTRegressOutCCdifference}
# regress the cell cycle difference - this worked
...
saveRDS(seurat.phase.ons762d, file = "./outputs50K/ONS76_2D/seurat.phase.regressedCCdiff.ons76.2d.rds")
```
--------------------------------------------------------------
...
```{r DimplotRegressOutCCdifferenceSCT}
seurat.phase.sct <- SCTransform(seurat.phase.sct,
                                   vars.to.regress = "CC.difference",
                                   method = "glmGamPoi", 
                                   verbose = FALSE)
...
```{r}
# normalise data
phase.ons762d.filtRiboMito.sct <- NormalizeData(filt.noRiboMito.ons76.2d)

# run cell cycle scoring
s.genes <- cc.genes$s.genes
g2m.genes <- cc.genes$g2m.genes
phase.ons762d.filtRiboMito.sct <- CellCycleScoring(phase.ons762d.filtRiboMito.sct,
                                 g2m.features = g2m.genes,
                                 s.features = s.genes)
```
...
phase.ons762d.filtRiboMito.sct <- SCTransform(phase.ons762d.filtRiboMito.sct,
                                   
                                   method = "glmGamPoi", 
                                   verbose = FALSE)

phase.ons762d.filtRiboMito.sct <- RunPCA(phase.ons762d.filtRiboMito.sct, features = c(s.genes, g2m.genes))

DimPlot(phase.ons762d.filtRiboMito.sct, reduction = "pca", group.by = "Phase")
```
...
phase.ons762d.filtRiboMito.sct$CC.difference <- phase.ons762d.filtRiboMito.sct$S.Score - phase.ons762d.filtRiboMito.sct$G2M.Score
