...
***Examine ONS76 monolayer cells that transcriptionally overlap the patient tumours strongly and compare their active pathways with those monolayer cells that do not overlap the patient tumours***

```{r eval=FALSE, echo=FALSE}
# Extract UMAP coordinates and cell identities
umap_coords <- as.data.frame(o76mono.hovShhMB.merged.clust@reductions$umap@cell.embeddings)
head(umap_coords, 3)

cell_metadata <- o76mono.hovShhMB.merged.clust@meta.data
```

```{r eval=FALSE, echo=FALSE}
# Combine UMAP coordinates with metadata (e.g., cell identity)
umap_coords$orig.ident <- cell_metadata$orig.ident
head(umap_coords, 3)
tail(umap_coords, 3)
```

```{r eval=FALSE, echo=FALSE}
# Define the region of interest (you can adjust the min/max values based on your outline)
# this should correspond roughly to the region of maximum overlap of patient tumours and ONS76 cells
x_min <- 0.5
x_max <- 6
y_min <- -0.3
y_max <- 1.5

selected_cells <- rownames(umap_coords[umap_coords$UMAP_1 > x_min & umap_coords$UMAP_1 < x_max & 
                                         umap_coords$UMAP_2 > y_min & umap_coords$UMAP_2 < y_max & 
                                         umap_coords$orig.ident == "ONS76_monolayer", ])
length(selected_cells) # 141 ONS76 cells
```

```{r eval=FALSE, echo=FALSE}
# Subset Seurat object to get only the 'red' ONS76 monolayer cells within the outlined region
subset_region <- subset(o76mono.hovShhMB.merged.clust, cells = selected_cells)
dim(subset_region)
```

```{r eval=FALSE, echo=FALSE}
# For comparison, subset the 'red' ONS76 monolayer cells well outside this region
x2_min <- 2
x2_max <- 6
y2_min <- -4
y2_max <- -1.3

non_selected_cells <- rownames(umap_coords[umap_coords$UMAP_1 > x2_min & umap_coords$UMAP_1 < x2_max & 
                                         umap_coords$UMAP_2 > y2_min & umap_coords$UMAP_2 < y2_max & 
                                         umap_coords$orig.ident == "ONS76_monolayer" & !rownames(umap_coords) %in% selected_cells, ])
subset_non_region <- subset(o76mono.hovShhMB.merged.clust, cells = non_selected_cells)
dim(subset_non_region)
```

```{r eval=FALSE, echo=FALSE}
# Add a new metadata column to differentiate the two sub-populations
subset_region$sub_population <- "region_selected"
subset_non_region$sub_population <- "region_non_selected"

# Now, merge the two Seurat objects. Both objects are derived from the same ONS76 monolayer sample so re-normalisation is not required
combined_seurat <- merge(subset_region, y = subset_non_region)

# Ensure the new metadata column is in place: it is
table(combined_seurat$sub_population)
```

Identify differentially expressed genes between region_selected and region_non_selected: do this using PRESTO and using FindMarkers() from Seurat
```{r eval=FALSE, echo=FALSE}
# Perform differential expression using PRESTO (note: use of the new metadata column). Seurat FindMarkers() tested and produces a list of ~100 genes. The top ranked genes all had negative values so this method was not used further.
presto_results <- wilcoxauc(combined_seurat, 'sub_population')

# Display the results for the specific group of interest (e.g., 'region_selected' vs others)
selected_vs_non_results <- presto_results %>%
  dplyr::filter(group == "region_selected") %>%
  dplyr::arrange(desc(logFC), desc(auc))
```

```{r eval=FALSE, echo=FALSE}
head(selected_vs_non_results, 30)
nrow(selected_vs_non_results)
```

```{r eval=FALSE, echo=FALSE}
# this code will only reveal UPregulated pathways (note: if logFC is higher than 0.1 too few genes are enriched)
subset_region_genesUP_presto <- presto_results %>%
  dplyr::filter(group == "region_selected" & logFC > 0.1) %>%
  arrange(desc(auc)) %>% 
  dplyr::select(feature, auc)

# how many genes?
nrow(subset_region_genesUP_presto)
```

```{r eval=FALSE, echo=FALSE}
# load msigdbr and use dplyr to get the appropriate gene set
...
```{r eval=FALSE, echo=FALSE}
# use PRESTO results first
ranks_selected_presto <- deframe(subset_region_genesUP_presto)
head(ranks_selected_presto)
```
...
fgseaRes.subset.region.GOBP <- 
  fgseaMultilevel(fgsea_sets_GOBP, stats = ranks_selected_presto, eps = 0, minSize = 15, scoreType = "pos", maxSize = 200)
```

```{r eval=FALSE, echo=FALSE}
# tidy up the data
fgseaResTidy.subset.region.GOBP <- fgseaRes.subset.region.GOBP %>%
  as_tibble() %>%
  arrange(desc(NES))

fgseaResTidy.subset.region.GOBP %>% 
  dplyr::select(-leadingEdge, -ES) %>% 
  arrange(padj) %>% 
  head(20)
```

```{r eval=FALSE, echo=FALSE, BarplotFGSEAgoBPSelectedRegionONS76monoPatientSHHMBclust, fig.height=20, fig.width=42}
# only plot the top 10 pathways
ggplot(fgseaResTidy.subset.region.GOBP %>% filter(padj < 0.05) %>% head(n=10), aes(reorder(pathway, NES), NES)) +
  geom_col(fill = "blue") +
  coord_flip() +
...
***Examine patient tumour cells that transcriptionally overlap the ONS76 monolayer cells strongly and compare their active pathways with those monolayer cells that do not overlap the patient tumours***
Similar pathways should be upregulated in the transcriptionally overlapping patient tumour cells and ONS76 monolayer cells
```{r eval=FALSE, echo=FALSE}
# Define the region of interest (you can adjust the min/max values based on your outline)
# this should correspond roughly to the region of maximum overlap of patient tumours and ONS76 cells
x_min <- 0.5
...
selected_cells <- rownames(umap_coords[umap_coords$UMAP_1 > x_min & umap_coords$UMAP_1 < x_max & 
                                         umap_coords$UMAP_2 > y_min & umap_coords$UMAP_2 < y_max & 
                                         umap_coords$orig.ident == "SHH-MB3", ])
length(selected_cells) # 37 patient tumour cells
```

```{r eval=FALSE, echo=FALSE}
# Subset Seurat object to get only the 'red' ONS76 monolayer cells within the outlined region
patient_subset_region <- subset(o76mono.hovShhMB.merged.clust, cells = selected_cells)
dim(patient_subset_region)
```
...
```{r eval=FALSE, echo=FALSE}
# Add a new metadata column to differentiate the two sub-populations
patient_subset_region$sub_population <- "pt_region_selected"
subset_non_region$sub_population <- "region_non_selected"

# Now, merge the two Seurat objects. Both objects are derived from the same ONS76 monolayer sample so re-normalisation is not required, and moreover, the o76mono.hovShhMB.merged.clust object, from which both patient tumour cells and ONS76 monolayer cells are derived, was also normalised during its generation.
combined_seurat_2 <- merge(patient_subset_region, y = subset_non_region)

# Ensure the new metadata column is in place: it is
table(combined_seurat_2$sub_population)
```

```{r eval=FALSE, echo=FALSE}
# Perform differential expression using PRESTO (note: use of the new metadata column). Seurat FindMarkers() tested and produces a list of ~100 genes. The top ranked genes all had negative values so this method was not used further.
presto_results_2 <- wilcoxauc(combined_seurat_2, 'sub_population')

# Display the results for the specific group of interest (e.g., 'region_selected' vs others)
selected_vs_non_results_2 <- presto_results_2 %>%
  dplyr::filter(group == "pt_region_selected") %>%
  dplyr::arrange(desc(logFC), desc(auc))

head(selected_vs_non_results_2, 3)
nrow(selected_vs_non_results_2)
```

```{r eval=FALSE, echo=FALSE}
# this code will only reveal UPregulated pathways (note: if logFC is higher than 0.1 too few genes are enriched)
subset_region_genesUP_presto_2 <- presto_results_2 %>%
  dplyr::filter(group == "pt_region_selected" & logFC > 0.25) %>%
  arrange(desc(auc)) %>% 
  dplyr::select(feature, auc)

# how many genes?
nrow(subset_region_genesUP_presto_2)
```
...
```{r eval=FALSE, echo=FALSE}
# use PRESTO results
ranks_selected_presto_2 <- deframe(subset_region_genesUP_presto_2)
head(ranks_selected_presto_2, 40)
```

```{r eval=FALSE, echo=FALSE}
# Step 1: Create a list of ribosomal genes
ribosomal_genes <- grep(pattern = "^RPS|^RPL", x = names(ranks_selected_presto_2), value = TRUE)

# Step 2: Filter the ranked list for ribosomal genes
ribosomal_ranks <- ranks_selected_presto_2[ribosomal_genes]
...
```{r eval=FALSE, echo=FALSE}
# Cross-reference with the full PRESTO results
ribosomal_genes_info <- selected_vs_non_results_2 %>%
  dplyr::filter(feature %in% ribosomal_genes)

# Display the information
ribosomal_genes_info
```

```{r eval=FALSE, echo=FALSE}
# use fgsea 'multilevel' function; the multilevel function has by default NPermSimple=1000
# use scoretype=pos for genes where logFC > 0
fgseaRes.subset.region.GOBP_2 <- 
  fgseaMultilevel(fgsea_sets_GOBP, stats = ranks_selected_presto_2, eps = 0, minSize = 15, scoreType = "pos", maxSize = 200)
```

```{r eval=FALSE, echo=FALSE}
# tidy up the data
fgseaResTidy.subset.region.GOBP_2 <- fgseaRes.subset.region.GOBP_2 %>%
  as_tibble() %>%
  arrange(desc(NES))

fgseaResTidy.subset.region.GOBP_2 %>% 
  dplyr::select(-leadingEdge, -ES) %>% 
  arrange(padj) %>% 
  head(20)
```

```{r eval=FALSE, echo=FALSE, BarplotFGSEAgoBPSelectedRegionPatientEnrichmentONS76monoPatientSHHMBclust, fig.height=20, fig.width=42}
ggplot(fgseaResTidy.subset.region.GOBP_2 %>% filter(padj < 0.05) %>% head(n=10), aes(reorder(pathway, NES), NES)) +
  geom_col(fill = "blue") +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
      title= "") + 
