...
```{r}
o76.3samples.allMarkers <- FindAllMarkers(o76.3samples.combined.sct.prepSct,
                                     assay = "SCT",
                                    verbose = FALSE,
                                    only.pos = TRUE,
                                    logfc.threshold = 0.25)
...
write.csv(o76.3samples.allMarkers.gene, 
          file = "./outputs50K/INTEGRATE/DEgenesUP.Integrated.ONS76Coculture.ONS76mono.ONS76Spher.csv",        quote = FALSE)
```

```{r}
# load the markers csv file
o76.3samples.allMarkers.gene <- 
  read.csv("outputs50K/INTEGRATE/DEgenesUP.Integrated.ONS76Coculture.ONS76mono.ONS76Spher.csv")
```
...
```{r HeatmapTop20genesONS76monoSpherO76onlyCocultSamplesIntegratedRNAassay, fig.height=10, fig.width=12}
...
```{r VlnPlotSox2Network6samplesMergedONS762d3dAndOns76onlyCocultClustersDAOY2d3d, fig.height=9, fig.width=12}
Idents(daoy.o76.cells.noDyCocult) <- "sample"
...
```{r BarchartCellCyclePhaseProportionsONS76MonoSpherCocultureIntegrated, fig.height=4, fig.width=3}
...
o76mono.g1cells <- 
  readRDS("outputs50K/ONS76_2D/G1phaseCells.ONS76monolayer.inputIntegration.SCTransformed.PCA.unclustered.rds")
o76spher.g1cells <- readRDS("outputs50K/ONS76_SPHER/G1phaseCells.ONS76spheroid.inputIntegration.SCTransformed.PCA.unclustered.rds")
...
```{r}
table(o76.g1.comb.sct$orig.ident) # mono=172, spher=668, cocult=172 which are the correct numbers of
                                  # cells in G1
```
...
o76.g1.comb.sct@meta.data[o76.g1.comb.sct@meta.data == "ons76cbo"] <- "ONS76_coculture_G1"
```
...
```{r}
nrow(o76.g1.comb.sct.allMarkers) # 3630 differentially expressed UP-regulated genes
```
...
```{r}
DefaultAssay(o76.g1.comb.sct) <- "SCT"  # in general not the best assay for integrated data - better
                                        # to use RNA assay, but the results for both assays
                                        # are very similar IN THIS CASE (see below code chunks)
```


```{r DotplotG1phaseGenesONS76IntegratedMonoSpherCocult, fig.height=4, fig.width=5}
...
```{r HeatmapSCTassayG1cellsONS76integratedMonoSpherCocult, fig.height=6, fig.width=8}
...
DoHeatmap(o76.g1.comb.sct.scaled, features = ons76.markers.g1, size = 10, assay = "SCT", group.by = "ident") + 
  guides(color = "none") +
  theme(text = element_text(size = 20), legend.title = element_text(size=20), legend.text=element_text(size=20))
```

```{r HeatmapRNAassayG1cellsONS76integratedMonoSpherCocult, fig.height=6, fig.width=8}
# when using the RNA assay make sure the object 'o76.g1.comb.sct' is freshly loaded from the saved (saveRDS)
# object and has NOT been through PrepSCTFindMarkers otherwise the heatmap will look odd!
...
ons76.markers.g1 <- c("DKK1", "ANKRD1", "FST", "BIRC5", "LYPD6B",
                      "ITM2B", "TIMP1", "IGFBP4", "IGFBP5", "CD59",
                      "NEUROD1", "DCX", "TUBB2B", "NR2F1", "KCNB2")
DefaultAssay(o76.g1.comb.sct) <- "RNA"
o76.g1.comb.sct.scaled.rna <- ScaleData(o76.g1.comb.sct, features = ons76.markers.g1)
DoHeatmap(o76.g1.comb.sct.scaled.rna, features = ons76.markers.g1, size = 10) + 
  guides(color = "none") +
  theme(text = element_text(size = 20), legend.title = element_text(size=20), legend.text=element_text(size=20))
```
