...
seurat.phase <- NormalizeData(Fullfilt.cbo.seurat)
```
...
# Plot the PCA colored by cell cycle phase - cell cycle for G2M vs S not obviously having an effect on clustering, as there is considerable overlap. There is perhaps some clustering/separation for G1 vs G2M phases. Not sure it is significant.
...
```{r}
saveRDS(seurat.phase, "./outputs50K/CBO/seurat.phase.cbo.rds")
```
