...
dy.3samples.combined.sct.subset <- 
  subset(dy.3samples.combined.sct, idents = c("DAOY_coculture", "DAOY_spheroid"))
...
top30 <- dyCocultVspher.allMarkers.gene.sorted %>% top_n(n = 30, wt = avg_log2FC) # wt = -p_val_adj is an alternative
```
...
dy.3samples.combined.sct.scaled <- ScaleData(dy.3samples.combined.sct, verbose = FALSE)
```


```{r HeatmapTop30genesDAOYmonoSpherDyOnlyCocultSamplesIntegratedRNAassay, fig.height=10, fig.width=12}
# Heatmap now uses all cells, no need to downsample
DoHeatmap(dy.3samples.combined.sct.scaled, features = top30$gene, size = 10, assay = "RNA", group.by = "ident") + 
  guides(color = "none") +
...
DoHeatmap(dy.3samples.combined.sct.scaled, features = features, size = 10, assay = "RNA", group.by = "ident") + 
  guides(color = "none") +
  theme(text = element_text(size = 20), legend.title = element_text(size=20), legend.text=element_text(size=15))
```
