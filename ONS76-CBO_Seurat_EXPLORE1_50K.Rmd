...
```
{r}
# check if the top genes in cluster 1 of o76cbo is represented in cbo. All the below genes
# the top markers are similar to the markers of cluster 1 in CBO sample.
cbo.markers %>% 
  filter(gene %in% c("SOX2", "SOX2-OT", "IGFBP2", "TTYH1", "ARX", "C1orf61", "HDAC9", "PTCHD1-AS"))
```

```
{r}
# According to clusterMap, ONS76-CBO cluster 1 has no counterpart cluster in CBO. Inspect this cluster
# manually to check:
# answer: yes cluster 1 is now annotated as 'RP-choroid plexus_like'
...
new.cluster.ids <- c("glia", "RP-choroid plexus_like", "RP-choroid plexus_1", 
                     "choroid plexus", "granule precursors", "ONS76_cluster_5", "purkinje-like",
                     "RL/DCN", "oligodendrocytes", "ONS76_cluster_9", "ONS76_cluster_10", "progenitors",
                    "ONS76_cluster_12")
...
metadata$celltype[which(metadata$seurat_clusters == "0")] <- "glia"
metadata$celltype[which(metadata$seurat_clusters == "1")] <- "RP-choroid plexus_like"
metadata$celltype[which(metadata$seurat_clusters == "2")] <- "RP-choroid plexus_1"
...
metadata$celltype[which(metadata$seurat_clusters == "5")] <- "ONS76_cluster_5"
metadata$celltype[which(metadata$seurat_clusters == "6")] <- "purkinje-like"
metadata$celltype[which(metadata$seurat_clusters == "7")] <- "RL/DCN"
metadata$celltype[which(metadata$seurat_clusters == "8")] <- "oligodendrocytes"
...
metadata$celltype[which(metadata$seurat_clusters == "11")] <- "progenitors"
metadata$celltype[which(metadata$seurat_clusters == "12")] <- "ONS76_cluster_12"
```
...
```
{r HeatmapONS76CboVSCboClusterCorrelations, fig.height=3, fig.width=3}
# FOR KNIT DO NOT RE-RUN THIS CODE CHUNK
...
***Extract the cells/clusters that are NeuroD1+***. 
There are 634 NEUROD1 positive cells - both healthy and cancerous
```
{r}
# FOR KNIT DO NOT RE-RUN THIS CODE CHUNK
# 392 NEUROD1+ cancer cells
neurod1.ons76cbo <- subset(o76cbo, idents = c("5", "9", "12")). # cluster 4 is non-malignant NEUROD1 cluster
...
```
{r}
# 235 NON-malignant NEUROD1+ granule precursors
table(o76cbo.annotate$seurat_clusters)
```

```
{r}
table(neurod1.ons76cbo$seurat_clusters) 
```
...
```
{r}
# subset o76cbo.annotate for all the malignant clusters
o76only.o76cboAnnot <- 
  subset(o76cbo.annotate, 
         idents = c("ONS76_cluster_5", "ONS76_cluster_9", "ONS76_cluster_10", "ONS76_cluster_12"))
```
...
new.cluster.ids.subset <- c("ONS76_cluster_5", "ONS76_cluster_9", "ONS76_cluster_10", "ONS76_cluster_11")
...
```
{r}
# ONS76-CBO, cancer cell numbers: clust 5 = 215, clust 9 = 116, clust 10 = 74, clust 12 = 60
table(o76only.o76cboAnnot.rescaled$seurat_clusters)
```

```
{r}
# save all the malignant clusters 5,9,10,11
...
------------------------------------------------------------------------------------------
***Gene expression in ONS76 cancer clusters 5, 9, 10, 11***
...
```{r ViolinPlotSox2networkONS76onlyONS76cbo, fig.width=14}
...
```{r ViolinPlotERRB4nrg3Meis1ZicONS76onlyONS76cbo, fig.width=14}
VlnPlot(o76only.o76cboAnnot.rescaled, 
        features = c("ERBB4", "NRG3", "MEIS1", "ZIC1", "ZIC2", "OLIG2"))
```
...
****Cluster 4 CBO control (granule precursors) - GO:BP gene set****
Test the gene set enrichment of cluster 4 in CBO, the granule cell cluster
```
{r}
# continue with preparing data for input to fgsea and getting bar charts of gene set 
# enrichment analysis
clust4.cbo.genes.presto <- cbo.presto %>%
  dplyr::filter(group == "4" & !(feature == "MALAT1")) %>%
...
```
{r}
ranks.clus4 <- deframe(clust4.cbo.genes.presto)
head(ranks.clus4)
```
...
fgseaRes.clus4.hallmark <- 
  fgseaMultilevel(fgsea_sets, eps = 0, stats = ranks.clus4, minSize = 15, scoreType = "pos", maxSize = 200)
```
...
fgseaResTidy.clus4.hallmark <- fgseaRes.clus4.hallmark %>%
  as_tibble() %>%
  dplyr::arrange(desc(NES))

fgseaResTidy.clus4.hallmark %>% 
...
```
{r}
saveRDS(fgseaResTidy.clus4.hallmark,
        file = "./outputs50K/CBO/granule.cluster4.cbo.multifgsea.presto.HallmarkGenes.rds")
```

```
{r}
fgseaResTidy.clus4.hallmark <- 
  readRDS("outputs50K/CBO/granule.cluster4.cbo.multifgsea.presto.HallmarkGenes.rds")
```
...
ggplot(fgseaResTidy.clus4.hallmark %>% filter(padj < 0.001) %>% head(n=20), aes(reorder(pathway, NES), NES)) +
...
****Cluster 5 ONS76-CBO - geneset enrichment (FGSEA) including input for g:Profiler and Cytoscape****
```{r}
# filter gene expression for cluster 5, the ONS76 cluster
...
ons76cbo.cl5.UPgenes.presto <- o76cbo.presto %>% 
  dplyr::filter(group == "5" & logFC > 0.25) %>%
  dplyr::arrange(desc(auc)) %>% 
  dplyr::select(feature, auc)

nrow(ons76cbo.cl5.UPgenes.presto) # genes=2640
```

```{r}
# FOR KNIT DO NOT RE-RUN THIS CODE CHUNK
# save as csv for input to g:Profiler (web app)
write.csv(ons76cbo.cl5.UPgenes.presto, 
          "./outputs50K/ONS76-CBO/ONS76cbo.cluster5.UPgenes.PRESTO.gProfilerInput.csv", 
          quote = F)
```
...
# continue with preparing data for input to fgsea and getting bar charts of gene set 
# enrichment analysis
clust5.genes.presto <- o76cbo.presto %>%
  dplyr::filter(group == "5") %>%
  arrange(desc(auc)) %>% 
  dplyr::select(feature, auc)
```
...
saveRDS(fgseaResTidy.clus5.hallmark,
        file = "./outputs50K/ONS76-CBO/cluster5.ons76cbo.multifgsea.presto.HallmarkGenes.rds")
```

```{r}
fgseaResTidy.clus5.hallmark <- 
  readRDS("outputs50K/ONS76-CBO/cluster5.ons76cbo.multifgsea.presto.HallmarkGenes.rds")
```
...
saveRDS(fgseaResTidy.clus5.GOBP, 
        file = "./outputs50K/ONS76-CBO/cluster5.ONS76Cbo.multifgsea.presto.GOBiolProcessGenes.rds")
```

```{r}
fgseaResTidy.clus5.GOBP <- 
  readRDS("outputs50K/DAOY-CBO/outputs50K/ONS76-CBO/cluster5.ONS76Cbo.multifgsea.presto.GOBiolProcessGenes.rds")
```


```{r BarplotFGSEAgoBPclust11DaoyCbo, fig.height=5, fig.width=6}
# only plot the top 20 pathways
ggplot(fgseaResTidy.clus5.GOBP %>% filter(padj < 0.001) %>% head(n=20), aes(reorder(pathway, NES), NES)) +
  geom_col(fill = "blue") +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
...
```{r}
# this code used to generate sorted gene list as input for gProfiler (web app). Unlikely to show
# gene set enrichment as there are only 72 genes differentially expressed in cluster 9
ons76cbo.cl9.UPgenes.presto <- o76cbo.presto %>% 
  dplyr::filter(group == "9" & logFC > 0.25) %>%
  dplyr::arrange(desc(auc)) %>% 
  dplyr::select(feature, auc)

nrow(ons76cbo.cl9.UPgenes.presto) # genes = 96 only
```
...
# continue with preparing data for input to fgsea and getting bar charts of gene set 
# enrichment analysis
clust9.genes.presto <- o76cbo.presto %>%
  dplyr::filter(group == "9") %>%
  arrange(desc(auc)) %>% 
  dplyr::select(feature, auc)
```
...
```{r}
# set eps to zero to get lower p-values
fgseaRes.clus9.hallmark <- 
  fgseaMultilevel(fgsea_sets, eps = 0, stats = ranks.clus9, minSize = 15, scoreType = "pos", maxSize = 200)
```

```{r}
# there are no significant gene sets with padj<0.05
fgseaResTidy.clus9.hallmark <- fgseaRes.clus9.hallmark %>%
  as_tibble() %>%
  dplyr::arrange(desc(NES))

fgseaResTidy.clus9.hallmark %>% 
  dplyr::select(-leadingEdge, -ES) %>% 
  dplyr::arrange(padj) %>% 
  head(10)
```
...
```{r}
fgseaResTidy.clus9.hallmark <- 
  readRDS("outputs50K/DAOY-CBO/outputs50K/ONS76-CBO/cluster9.ons76cbo.multifgsea.presto.HallmarkGenes.rds")
```


```{r BarplotFGSEAgoBPclust11DaoyCbo, fig.height=5, fig.width=6}
# only plot the top 20 pathways - NO ENRICHED PATHWAYS!!!
ggplot(fgseaResTidy.clus9.hallmark %>% filter(padj < 0.05) %>% head(n=20), aes(reorder(pathway, NES), NES)) +
  geom_col(fill = "blue") +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title= "GO:BP pathways NES from GSEA") + 
...
fgseaRes.clus9.GOBP <- 
  fgseaMultilevel(fgsea_sets_GOBP, stats = ranks.clus9, eps = 0, minSize = 15, scoreType = "pos", maxSize = 200)
```

```{r}
# tidy up the data
fgseaResTidy.clus9.GOBP <- fgseaRes.clus9.GOBP %>%
  as_tibble() %>%
  arrange(desc(NES))

fgseaResTidy.clus9.GOBP %>% 
  dplyr::select(-leadingEdge, -ES) %>% 
  arrange(padj) %>% 
  head(20)
```
...
saveRDS(fgseaResTidy.clus9.GOBP, 
        file = "./outputs50K/ONS76-CBO/cluster9.ONS76Cbo.multifgsea.presto.GOBiolProcessGenes.rds")
```

```{r}
fgseaResTidy.clus9.GOBP <- 
  readRDS("outputs50K/DAOY-CBO/outputs50K/ONS76-CBO/cluster9.ONS76Cbo.multifgsea.presto.GOBiolProcessGenes.rds")
```


```{r BarplotFGSEAgoBPclust9DaoyCbo, fig.height=5, fig.width=6}
# only plot the top 20 pathways - no enriched gene sets!
ggplot(fgseaResTidy.clus9.GOBP %>% filter(padj < 0.001) %>% head(n=20), aes(reorder(pathway, NES), NES)) +
  geom_col(fill = "blue") +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title= "GO:BP pathways NES from GSEA") + 
...
nrow(ons76cbo.cl10.UPgenes.presto) # genes = 381 only
```
...
saveRDS(fgseaResTidy.clus10.GOBP, 
        file = "./outputs50K/ONS76-CBO/cluster10.ONS76Cbo.multifgsea.presto.GOBPGenes.rds")
```

```{r}
# no need to plot - no significant genesets!
fgseaResTidy.clus10.GOBP <- 
  readRDS("outputs50K/ONS76-CBO/cluster10.ons76cbo.multifgsea.presto.GOBPGenes.rds")
```

```{r BarplotFGSEAgoBPclust10DaoyCbo, fig.height=5, fig.width=6}
# only plot the top 20 pathways - no enriched gene sets!
ggplot(fgseaResTidy.clus10.GOBP %>% filter(padj < 0.001) %>% head(n=20), aes(reorder(pathway, NES), NES)) +
  geom_col(fill = "blue") +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title= "GO:BP pathways NES from GSEA") + 
...
# this code used to generate sorted gene list as input for gProfiler (web app). Unlikely to show
# gene set enrichment as there are only 72 genes differentially expressed in cluster 9
ons76cbo.cl12.UPgenes.presto <- o76cbo.presto %>% 
  dplyr::filter(group == "12" & logFC > 0.25) %>%
  dplyr::arrange(desc(auc)) %>% 
  dplyr::select(feature, auc)

nrow(ons76cbo.cl12.UPgenes.presto) # genes = 6264
```

```{r}
# save as csv for input to g:Profiler (web app)
write.csv(ons76cbo.cl12.UPgenes.presto, 
          "./outputs50K/ONS76-CBO/ONS76cbo.cluster12.UPgenes.PRESTO.gProfilerInput.csv", 
          quote = F)
```
...
# continue with preparing data for input to fgsea and getting bar charts of gene set 
# enrichment analysis
clust12.genes.presto <- o76cbo.presto %>%
  dplyr::filter(group == "12") %>%
  arrange(desc(auc)) %>% 
  dplyr::select(feature, auc)
```
...
```{r}
ranks.clus12 <- deframe(clust12.genes.presto)
head(ranks.clus12)
```
...
fgseaRes.clus12.GOBP <- 
  fgseaMultilevel(fgsea_sets_GOBP, stats = ranks.clus12, eps = 0, minSize = 15, scoreType = "pos", maxSize = 200)
```
...
```{r}
saveRDS(fgseaResTidy.clus12.GOBP, 
        file = "./outputs50K/ONS76-CBO/cluster12.ONS76Cbo.multifgsea.presto.GOBPGenes.rds")
```

```{r}
fgseaResTidy.clus12.GOBP <- 
  readRDS("outputs50K/ONS76-CBO/cluster12.ons76cbo.multifgsea.presto.GOBPGenes.rds")
```

```{r BarplotFGSEAgoBPclust12DaoyCbo, fig.height=5, fig.width=6}
# only plot the top 20 pathways - no enriched gene sets!
