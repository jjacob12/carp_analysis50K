...
new.cluster.ids <- c("Roof Plate_1", "Purkinje cells_1", "Granule neurons", 
                     "ONS76_cluster_3", "RL-derived_1", "Ribosome cluster", "Roof plate_2",
...
metadata$celltype[which(metadata$seurat_clusters == "1")] <- "Purkinje cells_1"
...
```{r ViolinPlotMDM2Pou3f2Meis1HoxB2HoxBAS1ONS76cboSCT, fig.height=17, fig.width=22}
...
```{r eval=FALSE, echo=FALSE}
clust6.genes.cbo.presto <- cbo.presto %>%
...
```{r eval=FALSE, echo=FALSE}
nrow(clust6.genes.cbo.presto) # 962 genes
```


```{r eval=FALSE, echo=FALSE}
ranks.clus6 <- deframe(clust6.genes.cbo.presto)
head(ranks.clus6)
```
...
```{r eval=FALSE, echo=FALSE}
# gene sets are all associated with neuronal differentiation
...
ggplot(fgseaResTidy.clus6.cbo.GOBP %>% filter(padj < 0.05) %>% head(n=20), aes(reorder(pathway, NES), NES)) +
  geom_col(fill = "blue") +
  coord_flip() +
...
```{r eval=FALSE, echo=FALSE}
fgseaResTidy.clus2.GOBP <- 
  readRDS("outputs50K/DAOY-CBO/outputs50K/ONS76-CBO/cluster2.ONS76Cbo.multifgsea.presto.GOBiolProcessGenes.rds")
```


```{r eval=FALSE, echo=FALSE, BarplotFGSEAgoBPclust2ONS76cbo, fig.height=10, fig.width=15}
# only plot the top 20 pathways - no enriched gene sets!
ggplot(fgseaResTidy.clus2.GOBP %>% filter(padj < 0.05) %>% head(n=20), aes(reorder(pathway, NES), NES)) +
  geom_col(fill = "blue") +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
      title= "") + 
...
```{r eval=FALSE, echo=FALSE, BarplotFGSEAgoBPclust3ONS76cbo, fig.height=20, fig.width=15}
# only plot the top 20 pathways - no enriched gene sets!
...
```{r eval=FALSE, echo=FALSE, BarplotFGSEAgoBPclust10ons76Cbo, fig.height=20, fig.width=15}
# only plot the top 20 pathways
