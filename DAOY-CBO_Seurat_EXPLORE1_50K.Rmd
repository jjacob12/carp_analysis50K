...
new.cluster.ids <- c("glia", "choroid plexus", "RP-choroid plexus_1", "RL/DCN",
                     "granule precursors", "purkinje-like", "progenitors",
                     "DAOY_cluster_7", "RP-choroid plexus_2", "ciliated cells", "oligodendrocytes", "DAOY_cluster_11",
                     "DAOY_cluster_12")
...
metadata$celltype[which(metadata$seurat_clusters == "6")] <- "progenitors"
metadata$celltype[which(metadata$seurat_clusters == "7")] <- "DAOY_cluster_7"
metadata$celltype[which(metadata$seurat_clusters == "8")] <- "RP-choroid plexus_2"
metadata$celltype[which(metadata$seurat_clusters == "9")] <- "ciliated cells"
metadata$celltype[which(metadata$seurat_clusters == "10")] <- "oligodendrocytes"
metadata$celltype[which(metadata$seurat_clusters == "11")] <- "DAOY_cluster_11"
metadata$celltype[which(metadata$seurat_clusters == "12")] <- "DAOY_cluster_12"
```
...
# of this file. THIS FILE IS NOT USED AS CLUSTERMAP INPUT FILE.
...
```
{r HeatmapDaoyCboVSCboClusterCorrelations2, fig.height=3,fig.width=3}
# FOR KNIT DO NOT RE-RUN THIS CODE CHUNK
graphics.off()
par("mar")
par(mar=c(1,1,1,1))
...
Clusters 7, 11, 12. Only 1 NEUROD1+ cluster which is non-malignant.

```{r}
dyOnly.dyCboAnnotate <- subset(dyCbo.annotate, 
                               idents = c("DAOY_cluster_7",
                                          "DAOY_cluster_11",
                                          "DAOY_cluster_12"))
```
...
```{r}
new.cluster.ids.subset <- c("DAOY_cluster_7", "DAOY_cluster_11", "DAOY_cluster_12")
...
```{r}
# cluster 7 = 137 cells, cluster 11 = 73 cells, cluster 12 = 55 cells
table(dyOnly.dyCboAnnot.rescaled$seurat_clusters)
```

```{r}
# save all the malignant clusters 7,11,12
...
Extract the DAOY cells fro DAOY-CBO and save as a separate object

```
{r}
# FOR KNIT DO NOT RE-RUN THIS CODE CHUNK
daoyOnly.dyCboAnnot <- subset(dyCbo.annotate, 
                           idents = c("DAOY_cluster_7", "DAOY_cluster_11", "DAOY_cluster_12"))
```
...
```{r}
daoyOnly.dyCboAnnot.rescaled <- readRDS("outputs50K/DAOY-CBO/DaoyOnlyCells.SubsetDaoyCboSample.Annotated.Rescaled.SCT.NoRiboNoMito.CCdiffRegressed.rds")
```


```{r ViolinPlotSOX2NetworkDaoyOnlyDaoyCbo, fig.width=14}
VlnPlot(daoyOnly.dyCboAnnot.rescaled, 
        features = c("SOX2", "POU3F2", "FABP7", "OTX2", "ZIC2", "GLI2"))
```

```{r ViolinPlotSox2Erbb4DaoyOnlyDaoyCbo, fig.width=14}
VlnPlot(daoyOnly.dyCboAnnot.rescaled, 
        features = c("SOX2", "ERBB4", "FABP7", "OTX2", "GLI2"))
```
...
nrow(dyCbo.cl7.UPgenes.presto) # 2196 genes
```
...
write.csv(dyCbo.cl7.UPgenes.presto, 
          "./outputs50K/DAOY-CBO/daoyCbo.daoy.cluster7.UPgenes.presto.gProfilerInput.csv", quote = F)
```
...
fgseaRes.clus7.hallmark <- 
  fgseaMultilevel(fgsea_sets, eps = 0, stats = ranks.clus7, minSize = 15, scoreType = "pos", maxSize = 200)
```

```
{r}
fgseaResTidy.clus7.hallmark <- fgseaRes.clus7.hallmark %>%
  as_tibble() %>%
  arrange(desc(NES))

fgseaResTidy.clus7.hallmark %>% 
...
fgseaRes.clus7.reactome <- 
  fgseaMultilevel(fgsea_sets, eps = 0, stats = ranks.clus7, minSize = 15, scoreType = "pos", maxSize = 200)
```

```
{r}
fgseaResTidy.clus7.reactome <- fgseaRes.clus7.reactome %>%
  as_tibble() %>%
  arrange(desc(NES))

fgseaResTidy.clus7.reactome %>% 
  dplyr::select(-leadingEdge, -ES) %>% 
  arrange(padj) %>% 
  head(10)
```
...
ggplot(fgseaResTidy.clus7.reactome %>% filter(padj < 0.001) %>% head(n=20), aes(reorder(pathway, NES), NES)) +
  geom_col(aes(fill = NES > 0)) +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="Hallmark pathways NES from GSEA") + 
...
---------------------------------------------------------------------------------------------------------
***Cluster 11 - DAOY-CBO sample: gene set enrichment and g:Profiler input for cytoscape***
```
{r}
dyCbo.genes.presto %>%
...
clust11.genes.presto <- dyCbo.genes.presto %>%
  dplyr::filter(group == "11") %>%
  dplyr::arrange(desc(auc)) %>% 
  dplyr::select(feature, auc)
  
head(clust11.genes.presto)
```

```
{r}
ranks.clus11 <- deframe(clust11.genes.presto)
head(ranks.clus11)
```

```
{r}
dyCbo.cl11.UPgenes.presto <- dyCbo.genes.presto %>% 
  dplyr::filter(group == "11" & logFC > 0.25) %>%
  dplyr::arrange(desc(auc)) %>% 
  dplyr::select(feature, auc)

nrow(dyCbo.cl11.UPgenes.presto) # 600 genes
```

```
{r}
write.csv(dyCbo.cl11.UPgenes.presto, 
          "./outputs50K/DAOY-CBO/daoyCbo.cluster11.UPgenes.presto.gProfilerInput.csv", quote = F)
```
...
fgseaRes.clus11.hallmark <- 
  fgseaMultilevel(fgsea_sets, stats = ranks.clus11, eps = 0, scoreType = "pos", minSize = 15, maxSize = 200)
```
...
---------------------------------------------------------------------------------------------------
***Cluster 12 - DAOY-CBO sample: gene set enrichment and g:Profiler input for cytoscape***
```
{r}
dyCbo.genes.presto %>%
...
clust12.genes.presto <- dyCbo.genes.presto %>%
  dplyr::filter(group == "12") %>%
  arrange(desc(auc)) %>% 
  dplyr::select(feature, auc)
  
head(clust12.genes.presto)
```
...
```
{r}
dyCbo.cl12.UPgenes.presto <- dyCbo.genes.presto %>% 
  dplyr::filter(group == "12" & logFC > 0.25) %>%
  dplyr::arrange(desc(auc)) %>% 
  dplyr::select(feature, auc)

nrow(dyCbo.cl12.UPgenes.presto) # 7027 genes as input for gProfiler
```

```
{r}
write.csv(dyCbo.cl12.UPgenes.presto, 
          "./outputs50K/DAOY-CBO/daoyCbo.cluster12.UPgenes.presto.gProfilerInput.csv", quote = F)
```
...
```{r}
fgseaResTidy.clus12.GOBP <- readRDS("outputs50K/DAOY-CBO/daoy_clus12.daoyCbo.multifgsea.HallmarkGenes.rds")
```
